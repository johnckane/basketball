...
```{r}
?kmeans
k2 <- kmeans(churn_data[,-1],2)
k3 <- kmeans(churn_data[,-1],3)
k4 <- kmeans(churn_data[,-1],4)
```
...
```{r}
ggplot(data= churn_data %>% mutate(current_year = ifelse(Season == 2019,'Yes','No')),
       aes(x = k4,y=Season,color = current_year)) +
  geom_point()
```
